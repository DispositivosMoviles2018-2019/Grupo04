package ec.edu.uce.final_2h_g04.modelo;
...
public class BackupTask  extends AsyncTask<String,Void,Integer> {
    ...
    protected Integer doInBackground(String... params) {
        ...
        File dbFile = mContext.getDatabasePath( Environment.getExternalStorageDirectory().getAbsolutePath() + "/MIS_DATOS_VEHICULOS/" +DB_NAME);


        //File dbFile = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/" +"respaldo_db");


        //Get a reference to the directory location for the backup
        File exportDir =
                new File(Environment.getExternalStorageDirectory().getAbsolutePath(), "MIS_DATOS_VEHICULOS");
        if (!exportDir.exists()) {
            exportDir.mkdirs();
        }
        File backup = new File(exportDir, dbFile.getName()+"_respaldo");
        //File backup = new File(exportDir ,"respaldo_db");
        ...
        if(command.equals(COMMAND_BACKUP)) {
            ...
            try {
                ...
                //fileCopy(backup, new File(exportDir, dbFile.getName()+"_restaurado"));
